...
class DExpertsLlama:
    ...
    def forward(
        ...
        if pos_1_inputs != None:
            pos_1_outputs = self.model(**pos_1_inputs, return_dict=return_dict)
            neg_1_outputs = self.model(**neg_1_inputs, return_dict=return_dict)
            pos_2_outputs = self.model(**pos_2_inputs, return_dict=return_dict)
            neg_2_outputs = self.model(**neg_2_inputs, return_dict=return_dict)

            return base_outputs, pos_1_outputs, neg_1_outputs, pos_2_outputs, neg_2_outputs
        return base_outputs
    ...
    def generate(
        ...
        pos_1_input_ids: Optional[torch.Tensor] = None,
        pos_2_input_ids: Optional[torch.Tensor] = None,
        neg_1_input_ids: Optional[torch.Tensor] = None,
        neg_2_input_ids: Optional[torch.Tensor] = None,
        base_attention_mask: Optional[torch.Tensor] = None,
        pos_1_attention_mask: Optional[torch.Tensor] = None,
        pos_2_attention_mask: Optional[torch.Tensor] = None,
        neg_1_attention_mask: Optional[torch.Tensor] = None,
        neg_2_attention_mask: Optional[torch.Tensor] = None,
        method = None,
        weight_method = None,
        ...
        **kwargs
        ...
        pos_1_kwargs = kwargs.copy() 
        pos_2_kwargs = kwargs.copy() 
        neg_1_kwargs = kwargs.copy()
        neg_2_kwargs = kwargs.copy()
        ...
        pos_1_input_ids = pos_1_input_ids.to(pos_1_input_ids.device)
        pos_1_kwargs['attention_mask'] = pos_1_attention_mask
        pos_2_input_ids = pos_2_input_ids.to(pos_2_input_ids.device)
        pos_2_kwargs['attention_mask'] = pos_2_attention_mask
        neg_1_input_ids = neg_1_input_ids.to(neg_1_input_ids.device)
        neg_1_kwargs['attention_mask'] = neg_1_attention_mask
        neg_2_input_ids = neg_2_input_ids.to(neg_2_input_ids.device)
        neg_2_kwargs['attention_mask'] = neg_2_attention_mask
        ...
        for step in range(max_new_tokens):
            ...
            if cal:
                pos_1_inputs = self.model.prepare_inputs_for_generation(pos_1_input_ids, **pos_1_kwargs)
                pos_2_inputs = self.model.prepare_inputs_for_generation(pos_2_input_ids, **pos_2_kwargs)
                neg_1_inputs = self.model.prepare_inputs_for_generation(neg_1_input_ids, **neg_1_kwargs)
                neg_2_inputs = self.model.prepare_inputs_for_generation(neg_2_input_ids, **neg_2_kwargs)
                
            # DExperts
                base_outputs, pos_1_outputs, neg_1_outputs, pos_2_outputs, neg_2_outputs = self.forward(
                    base_inputs, pos_1_inputs, neg_1_inputs, pos_2_inputs, neg_2_inputs, return_dict=True
                )
                base_next_token_logits = base_outputs.logits[..., -1, :]
                pos_1_next_token_logits = pos_1_outputs.logits[..., -1, :]
                neg_1_next_token_logits = neg_1_outputs.logits[..., -1, :]
                pos_2_next_token_logits = pos_2_outputs.logits[..., -1, :]
                neg_2_next_token_logits = neg_2_outputs.logits[..., -1, :]

                # sometimes our experts have extra (irrelevant) tokens at the end of the normal vocabulary
                pos_1_next_token_logits = pos_1_next_token_logits[:, :base_next_token_logits.shape[-1]]
                neg_1_next_token_logits = neg_1_next_token_logits[:, :base_next_token_logits.shape[-1]]
                pos_2_next_token_logits = pos_2_next_token_logits[:, :base_next_token_logits.shape[-1]]
                neg_2_next_token_logits = neg_2_next_token_logits[:, :base_next_token_logits.shape[-1]]
                # DExperts!
                ...
                if weight_method == "entropy":
                    next_token_logits = (
                        ...
                        entropy_base * (pos_1_next_token_logits - neg_1_next_token_logits + pos_2_next_token_logits - neg_2_next_token_logits)
                    )
                elif weight_method == "alpha":
                    next_token_logits = (
                        base_next_token_logits +
                        self.alpha * (pos_1_next_token_logits - neg_1_next_token_logits + pos_2_next_token_logits - neg_2_next_token_logits)
                    )
                else:
            ...
            pos_1_input_ids = torch.cat([pos_1_input_ids, next_tokens[:, None]], dim=-1)
            neg_1_input_ids = torch.cat([neg_1_input_ids, next_tokens[:, None]], dim=-1)
            pos_2_input_ids = torch.cat([pos_2_input_ids, next_tokens[:, None]], dim=-1)
            neg_2_input_ids = torch.cat([neg_2_input_ids, next_tokens[:, None]], dim=-1)

            # update kwargs
            base_kwargs = self._update_model_kwargs_for_generation(base_outputs, base_kwargs)
            pos_1_kwargs = self._update_model_kwargs_for_generation(pos_1_outputs, pos_1_kwargs)
            neg_1_kwargs = self._update_model_kwargs_for_generation(neg_1_outputs, neg_1_kwargs)
            pos_2_kwargs = self._update_model_kwargs_for_generation(pos_2_outputs, pos_2_kwargs)
            neg_2_kwargs = self._update_model_kwargs_for_generation(neg_2_outputs, neg_2_kwargs)
